...
export default function CourseCard({ course }: { course: Course }) {
    return (
        <article className="relative flex w-full max-w-sm flex-col overflow-hidden rounded-2xl border border-gray-300 bg-white shadow-lg transition-all hover:scale-[1.02] hover:shadow-xl dark:border-gray-700 dark:bg-gray-900 px-2 py-3">
            {/* Contenido */}
            ...
            <div className="flex flex-col px-6">
                    ...
                    {course.promedio > 0 ? (
                        Array.from({ length: 5 }).map((_, i) => (
                            <Star
                                key={i}
                                size={18}
                                fill={i < Math.round(course.promedio * 5) ? "currentColor" : "none"}
                            />
                        ))
                    ) : (
                ...
                <a
                    href={`/${course.sigle}`}
                    className="mt-4 flex items-center justify-center rounded-lg bg-blue-600 px-4 py-2 text-white transition-all hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-300 dark:bg-blue-500 dark:hover:bg-blue-600 dark:focus:ring-blue-700"
                >
                    Ver más detalles
                </a>
            </div>
