...
export default function PageTitle({ children, size = "md", icon = false }: PageTitleProps) {
  ...
  return (
    <div className="flex items-center space-x-4">
      ...
      <h1
        className={`font-bold text-gray-900 dark:text-white ${sizeClasses[size]} relative transition-all duration-300 hover:text-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:hover:text-blue-400`}
      >
        {children}
        <span className="absolute bottom-0 left-0 h-[2px] w-full scale-x-0 bg-gradient-to-r from-blue-500 to-teal-400 transition-all duration-300 group-hover:scale-x-100" />
      </h1>
    </div>
  );
}
