...
function formatPrerequisiteGroup(group: any): string {
	...
	if (group.courses && group.courses.length > 0) {
		const courseTexts = group.courses.map(
			(course: any) => `${course.sigle}${course.isCoreq ? '(c)' : ''}`
		)
	...
	if (group.groups && group.groups.length > 0) {
		const groupTexts = group.groups.map((subGroup: any) => `(${formatPrerequisiteGroup(subGroup)})`)
