...
export function findConflictResolution(
  ...
  for (const conflictingCourse of conflictingCourses) {
    ...
    for (const alternativeSection of availableSections) {
      ...
      if (!hasScheduleConflicts(testCourses, courseSections)) {
        ...
        const courseInfo = courseOptions.find(opt => opt.id === `${courseId}-${alternativeSection}`);
  ...
  return {
    ...
    message: "No se pudieron encontrar secciones alternativas que resuelvan los conflictos. Es posible que necesites eliminar algunos cursos."
  };
}
