...
export function SelectInput({
	...
	return (
		<div className={cn('space-y-2', className)} {...props}>
			{label && (
				<label htmlFor={selectProps?.id} className="text-foreground block text-sm font-medium">
					{label}
			...
			<div className="relative">
				...
				<select
					className={cn(
						'focus:ring-opacity-50 bg-background w-full appearance-none rounded-md border transition-colors focus:ring',
				...
				<div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
					<svg className="fill-muted-foreground h-4 w-4" viewBox="0 0 20 20">
						<path
							fillRule="evenodd"
			...
			{description && !hasError && <p className="text-muted-foreground text-xs">{description}</p>}
