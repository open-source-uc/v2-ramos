...
const iconVariants = cva('fill-muted-foreground', {
	variants: {
		size: {
			sm: 'h-3.5 w-3.5',
			default: 'h-4 w-4',
...
const NumericInput = React.forwardRef<HTMLInputElement, NumericInputProps>(
	(
		...
		const describedBy =
			[description ? descriptionId : null, errorMessage ? errorId : null, ariaDescribedby]
				.filter(Boolean)
				.join(' ') || undefined
			...
			<div className="space-y-2">
				...
				{label && (
					<label
						id={labelId}
						htmlFor={inputId}
						className={cn('text-foreground block text-sm font-medium', disabled && 'opacity-50')}
					>
						{label}
						{required && (
				...
				{description && (
					<p
						id={descriptionId}
						className={cn('text-muted-foreground text-xs', disabled && 'opacity-50')}
					>
						{description}
					</p>
				)}
