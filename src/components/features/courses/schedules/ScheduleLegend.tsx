...
export function ScheduleLegend({
	...
	const validTypes = [
		...new Set(
			typesToShow.map((type) => normalizeClassType(type)).filter((type) => type in CLASS_TYPES)
		),
	]
	...
	return (
		<div className={cn('grid gap-3', gridCols)}>
			{validTypes.map((type) => {
				...
				const colorVariant =
					CLASS_TYPE_COLORS[type as keyof typeof CLASS_TYPE_COLORS] || 'schedule_blue'
				...
				return (
					<div key={type} className="flex items-center gap-2 text-sm">
						<Pill
							...
							size="xs"
							className="tablet:text-xs min-w-0 px-2 py-1 text-[10px]"
						>
							{classInfo.short}
						</Pill>
